...
// number k and omegae. It is written for root finding purpose where the "wavenumber" function
...
//
// So: omegae = sqrt(gk*tanh(kh)) - k*U*cos(beta)
//
// And the function's return value for a specific k and omegae is:
//
// [ sqrt( g*k*tanh(k*depth) ) - omegae - U*k*cos(beta) ]
//
// The idea is to find the k, which is the root of above equation for an omegae
//
// For the zero speed problem this turns simply to omegae - sqrt(gk*tanh(kh)) = 0;

#include "OW3DUtilFunctions.h"

double OW3DSeakeeping::Dispersion(
    double k,
    void *params)
{
  ...
  return (sqrt(g * k * tanh(k * depth)) - omegae - U * k * cos(beta));
}
