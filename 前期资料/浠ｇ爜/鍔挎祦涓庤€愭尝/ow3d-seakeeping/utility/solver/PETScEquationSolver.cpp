...
//==================================================================================
{
  ...
  if (Amx != NULL)
  {
    ierr = MatGetInfo(Amx, MAT_GLOBAL_SUM, &matInfo);
    CHKERRQ(ierr);
  }
...
//==================================================================================
{
  ...
  {
    if (Oges::debug & 2)
      printf("Initialize PETSc, oges.argc=%i...\n", oges.argc);
    ...
    if (turnOnPETScMemoryTracing)
    {
      ...
      ierr = PetscMemoryGetMaximumUsage(&space);
      CHKERRQ(ierr);
...
//   KSPPREONLY      = Use only the preconditioner
{
  ...
  if (parameters.solverMethod != solverMethod)
  {
    ...
    ierr = KSPSetType(ksp, krylovSpaceMethod);
    CHKERRQ(ierr);
  ...
  if (parameters.preconditioner != preconditioner)
  {
    ...
    ierr = PCSetType(pc, petscPreconditioner);
    CHKERRQ(ierr);
      ...
      parameters.preconditioner == OgesParameters::incompleteLUPreconditioner)
  {
    ...
    if (parameters.preconditioner == OgesParameters::incompleteLUPreconditioner) // ******* fix other cases *****
    {
      ...
      ierr = PCFactorSetMatOrderingType(pc, matOrdering);
      CHKERRQ(ierr);
#else
      ierr = PCFactorSetMatOrdering(pc, matOrdering);
      CHKERRQ(ierr);
#endif
  ...
  {
    ierr = KSPGMRESSetRestart(ksp, parameters.gmresRestartLength);
    CHKERRQ(ierr);
  ...
  double atol = parameters.absoluteTolerance > 0. ? parameters.absoluteTolerance : max(Real(numberOfEquations), 500.) * REAL_EPSILON;
      ...
      parameters.preconditioner == OgesParameters::incompleteLUPreconditioner)
  {
    ...
    ierr = PCFactorSetLevels(pc, parameters.numberOfIncompleteLULevels);
    CHKERRQ(ierr);
    ierr = PCFactorSetFill(pc, parameters.incompleteLUExpectedFill);
    CHKERRQ(ierr);
...
// ======================================================================================================
{
  ...
  if (Oges::debug & 2)
    cout << "...Preconditioner\n";
  ierr = setupPreconditioner(ksp, brhs, xsol);
  CHKERRQ(ierr); // see below for this routine
  ...
  if (copyOfSolutionNeeded || (matrixFormat != Oges::other && parameters.rescaleRowNorms))
  {
    ierr = VecRestoreArray(xsol, &soln);
    CHKERRQ(ierr);
  }
...
// =================================================================================
{
  ...
  if (matrixFormat == Oges::other)
  {
    ...
    ierr = MatAssemblyBegin(Amx, MAT_FINAL_ASSEMBLY);
    CHKERRQ(ierr);
    ierr = MatAssemblyEnd(Amx, MAT_FINAL_ASSEMBLY);
    CHKERRQ(ierr);
  ...
  else
  {
    ...
    if (!isMatrixAllocated)
    {
      ...
      while (numberOfEquations % parameters.blockSize != 0)
      ...
      {
        if (Oges::debug & 2)
          printF("PETScEquationSolver:Using -mat_block_size option: block size = %i\n"
      ...
      else
      {
        ...
        if (Oges::debug & 2)
          printF("PETScEquationSolver:Using blockSize=%i from OgesParameters\n", blockSize);
      }

      preallocRowStorage(blockSize);
      assert(nzzAlloc != NULL);

      if (blockSize == 1)
      {
        ierr = MatCreateSeqAIJ(PETSC_COMM_SELF, neq, neq, numberOfNonzeros, nzzAlloc, &Amx);
        CHKERRQ(ierr);
      ...
      else
      {
        ...
        int nz = 0; // not used ?
        ierr = MatCreateSeqBAIJ(PETSC_COMM_SELF, blockSize, neq, neq, nz, nzzAlloc, &Amx);
        CHKERRQ(ierr);
      }
      ...
      ierr = VecCreateSeq(PETSC_COMM_SELF, neq, &xsol);
      CHKERRQ(ierr);
      ierr = VecCreateSeq(PETSC_COMM_SELF, neq, &brhs);
      CHKERRQ(ierr);
    ...
    for (irowm1 = 0; irowm1 < neq; irowm1++)
    {
      ...
      for (j = ia_[irowm1]; j <= ia_[irowm1 + 1] - 1; j++)
      {
        v = dsc * aval[j - 1];
        if (fabs(v) < eps)
          continue; // *wdh* May 23, 2015 -- to catch small user supplied eqn coeffs

        jcolm1 = ja_[j - 1] - 1;
        // rownorm = rownorm+ fabs(v);
        // To matrix Amx, Insert row=irow, col=jcol, INSERT (NEW) VALUE
        ierr = MatSetValues(Amx, 1, &irowm1, 1, &jcolm1, &v, insrtOrAdd);
        CHKERRQ(ierr);
  ...
  if (Oges::debug & 4)
  {
    ...
    ierr = MatView(Amx, PETSC_VIEWER_STDOUT_WORLD);
    CHKERRQ(ierr);
  }
...
// ===========================================================================================================
{
  ...
  if (!oges.solvingSparseSubset)
  {
    ierr = oges.formRhsAndSolutionVectors(u, f);
    assert(ierr == 0);
  }
  ...
  if (copyOfSolutionNeeded || (matrixFormat != Oges::other && parameters.rescaleRowNorms))
  {
    ...
    {
      v = ovSol[i];
      ierr = VecSetValues(xsol, 1, &i, &v, INSERT_VALUES);
      CHKERRQ(ierr);

      v = ovRhs[i] * dscale[i]; // SCALE rhs as the matrix!!
      ierr = VecSetValues(brhs, 1, &i, &v, INSERT_VALUES);
      CHKERRQ(ierr);
  ...
  else
  {
    ...
    ierr = VecCreateSeqWithArray(comm, blockSize, n, ovRhs, &brhs);
    CHKERRQ(ierr);
    ierr = VecCreateSeqWithArray(comm, blockSize, n, ovSol, &xsol);
    CHKERRQ(ierr);
#endif
  }
  ...
  CHKERRQ(ierr);
  ierr = VecAssemblyBegin(brhs);
  CHKERRQ(ierr);
  ierr = VecAssemblyEnd(xsol);
  CHKERRQ(ierr);
  ierr = VecAssemblyEnd(brhs);
  CHKERRQ(ierr);
...
// ===============================================================
{
  ...
  if (!isMatrixAllocated)
  {
    ...
    ierr = MatCreateSeqAIJ(comm, N, N, nda / N + 1 + 1 + 1 + 1, PETSC_NULL, &Amx);
    CHKERRQ(ierr);
...
// ===============================================================
{
  ...
  j--;
#ifdef OV_USE_DOUBLE
  ierr = MatSetValues(Amx, 1, &i, 1, &j, &val, INSERT_VALUES);
  CHKERRQ(ierr);
#else
  PetscScalar sval = val;
  ierr = MatSetValues(Amx, 1, &i, 1, &j, &sval, INSERT_VALUES);
  CHKERRQ(ierr);
#endif
    ...
    setupPreconditioner(KSP ksp00, Vec brhs00, Vec xsol00)
{
  ...
  if (isDHPreconditioner)
  {
    ...
    if (parameters.incompleteLUTypeInDH)
    {
      ...
      CHKERRQ(ierr);
      ierr = PCShellSetApply(pc, MyILU_apply, (void *)dh_ctx);
      CHKERRQ(ierr);

      /* "extracts" (i.e, copies) matrix from A, and
         optionally applies sparcification */
      ierr = MyILU_setup(dh_ctx, Amx);
      CHKERRQ(ierr);

      dh_start();
      ierr = MyILU_factor(dh_ctx);
      CHKERRQ(ierr);
      dh_stop();
    ...
    {
      dh_start();
      ierr = SLESSetUp(sles00, brhs00, xsol00);
      CHKERRQ(ierr);
      dh_stop();
      dh_setupTime = dh_getCpuTime();
    }
...
void PETScEquationSolver::dh_computeResidualReduction(double &residReduction)
{
  ...
  Vec residVec;
  int ierr;
  ierr = VecDuplicate(brhs, &residVec);
  CHKERRQ(ierr);
  ierr = VecNorm(brhs, NORM_2, &b_norm);
  CHKERRQ(ierr); /* starting residual norm, since x_init=0 */
  ierr = MatMult(Amx, xsol, residVec);
  CHKERRQ(ierr);
  ierr = VecAXPY(&neg_one, brhs, residVec);
  CHKERRQ(ierr); /* u is final residual */
  ierr = VecNorm(residVec, NORM_2, &r_norm);
  CHKERRQ(ierr);
