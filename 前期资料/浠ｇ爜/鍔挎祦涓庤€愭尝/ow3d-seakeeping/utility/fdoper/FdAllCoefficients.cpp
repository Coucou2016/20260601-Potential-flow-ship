...
RealArray OW3DSeakeeping::FdAllCoefficients(unsigned int a, unsigned int b)
{
  unsigned int i, j;

  // build coefficient matrix

  unsigned int stenc_size = a + b + 1;

  RealArray results(stenc_size, stenc_size);
  Range r(0, stenc_size - 1);

  gsl_matrix *m = gsl_matrix_alloc(stenc_size, stenc_size);

  for (i = 0; i < stenc_size; i++)
  {
    for (j = 0; j < stenc_size; j++)
    {
      if (i < a)
      {
  ...
  int signum;
  gsl_matrix *inverse = gsl_matrix_alloc(stenc_size, stenc_size);
  gsl_permutation *perm = gsl_permutation_alloc(stenc_size);
  gsl_linalg_LU_decomp(m, perm, &signum);
  gsl_linalg_LU_invert(m, perm, inverse);

  for (i = 0; i < stenc_size; i++) // for this derivative
  {
    for (j = 0; j < stenc_size; j++) // all coefficients
    {
