...
![](images/01d53de6a43a99c2628f04b5c20d453bf871bce55ebdeae8524ff1f1cab987a9.jpg)
...
![](images/968c97f4d033e0fb17b662376135ad4d600d953d027ce210c7b3202fb775e897.jpg)
...
```mermaid
graph TD
    A["Flexible solar panel"] --> B["Charge controller"]
    B --> C["Inverter"]
    C --> D["AC Appliance"]
    B --> E["Solar battery"]
    E --> F["DC Appliance"]
...
where:  = angle of deadrise of planing surface

= trim angle of planing area

![](images/9a581970209b34a87998a7fd623e9037f9ec314dcf453d0448b892e475363d95.jpg)
...
![](images/f756d06dcfc4503c9d993e4f4185631fa2d154200ea05c5675d904d2db860ab6.jpg)
...
```mermaid
graph TD
    A["Start"] --> B["Read in V,Δ, LCG, LOA, LWL, B and β"]
    B --> C["Compute C_V = (V/√gb) and C_Lβ = (gΔ/1/2ρV²b²)"]
    C --> D["For i = 1"]
    D --> E["C_LO1 = 0.1"]
    E --> F["Compute f = C_LOi - 0.0065βC_LOi^0.6 - C_Lβ; f̂ = 1 - 0.0039βC_LOi^-0.4; C_LOi+1 = C_LOi - f/f'"]
    F --> G{If C_LOi+1 - C_LOi ≤ 0.0001}
    G -->|NO| H["I=i+1"]
    H --> I["I=i+1"]
    I --> J{If λ_i+1 - λ_i ≤ 0.0001}
    J -->|Yes| K["C_LO = C_LOi+1"]
    K -->|λ_i+1=4.0| L["Compute g = 0.7925Bλ_i³ - 2.39LCGλ_i² + 3.9075BC_v²λ_i - 5.21 LCGC_v²; g' = 2.3775Bλ_i² - 4.78 LCGλ_i + 3.9075 BC_v²; and λ_i + 1 = λ_i - g/j'"]
    L --> M{If λ_i+1 - λ_i ≤ 0.0001}
    M -->|No| N["I=i+1"]
    N --> O["I=i+1"]
    O --> P{If λ_i+1 - λ_i ≤ 0.0001}
    P -->|Yes| Q["C_LO = C_LOi+1"]
    Q --> R["Compute T = 1.1√(C_LO/0.012λ^(1/2) + 0.0055λ^(5/2)/C_V^2); λ_K = λ - 0.03 + ½(0.57 + β/1000)(tan β/2 tan τ - β/67)); R_n = Vλb/ø; CF = 0.075/(log R_n - 2) + 0.0004; V_1/V = √(1 - 0.0120τ^(1.1)/λ^(1/2) cos τ; R_T/W = tan τ + (V_1/V)^2 C_F / C_Lβ cos τ cos β)<br>END"]
```
</details>
...
$$
\lambda_ {k \beta} = \frac {\lambda^ {0 . 8}}{\cos \beta} (1 - 0. 2 9 (\sin \beta) ^ {0. 2 8}) \times \left(1 - 1. 3 5 (\sin \beta) ^ {0. 4 4} \frac {m _ {\Delta}}{\sqrt {C v}}\right) \tag {7}
$$
...
where: $m _ { \Delta } = \frac { \displaystyle \frac { 0 . 7 \pi \lambda } { 1 + 1 . 4 \lambda } \left( 0 . 7 5 + 0 . 0 8 \frac { \lambda ^ { 0 . 2 8 } } { \sqrt { C \nu } } \right) + \frac { \lambda - 0 . 8 } { 3 \lambda + 1 . 2 C \nu ^ { 2 } } } { \displaystyle \frac { 0 . 7 \pi \lambda } { 1 + 1 . 4 \lambda } + \frac { \lambda - 0 . 4 } { \lambda + 0 . 4 } \left( \frac { \lambda } { { C \nu } ^ { 2 } } \right) }$ 0.28  

The hydrodynamic moment factor $( ) m _ { \Delta }$ is a necessary correction for the wetted length ratio which is absent in Savitsky model. Details about CAHI method are presented [19], however, for dimensional total resistance variables such as: the speed , displacement $\Delta ,$ chine beam , Deadrise angle $\beta ,$ longitudinal center of gravity LCG, mean wetted length ratio , and the trim τ are identified
...
where $E _ { e s t } =$ Estimated energy storage, $E _ { d } = \mathrm { A v e r a g e }$ energy demand, $E _ { a u t } = \mathrm { D a y }$ r of autonomy
...
$$
E _ {s a f e} = \frac {E _ {e s t}}{D O D} \tag {12}
$$

![](images/3ca74395c2a2b15c2f35dde3c03d514c8c5fb806103b46fe9885bd37844421d6.jpg)
...
```mermaid
graph TD
    A["Start"] --> B["Read in V,Δ, LCG, LOA, LWL, B, b and β"]
    B --> C["Compute C_V = V/√gb; mΔ = LCG/b and C_Lβ = gΔ/1/2ρV²b²"]
    C --> D["For i=1"]
    D --> E["K = (0.7πλ_i/1+1.4λ_i(0.75+0.08λ_i^0.865/√C_V)+λ_i-0.08λ_i^2/3λ_i+1.2C_V^2)/(0.7πλ_i/1+1.4λ_i+λ_i-0.4λ_i/λ_i+0.4C_V^2)"]
    E --> F{C= K-mJΔ: -0.01≤C≥0.001}
    F -->|c>0.01| G["λ_1=λ_1-0.0001"]
    F -->|c<0.01| H["λ_i=λ_i+0.0001"]
    G --> I["λ_1=λ_1-0.0001"]
    H --> J["End"]
    I --> K["Compute T = CL/(0.7πλ_i/1+1.4λ_i + λ_i-0.4λ_i)/λ_i+0.4C_V^2"]
    K --> L["λ_β = λ / cos β 0.8 [1 - 0.29(sin β)^0.28"]X["1 + 1.35(sin β)^0.44 X/mΔ/√C_V"]; R_n = Vλb/σ; τ_β = T + 0.15(sin β)^0.8/C_v X(1 - 0.17√λβ cos β)/√λβ cos β; C_f = 0.075/(log R_n - 2)^2 + 0.0004; V_m = V["1 - τ/1-λ"]; S = b^2λβ/cos β and R_T = Δ tan τ + 0.5C_fSρV_m^2/cos τβ]
    J --> M["END"]
```
</details>
...
$E _ { s a f e } =$ Safe energy storage,  = Depth of Discharge
...
$\eta _ { i } =$ Inverter efficiency, $\eta _ { c } = \mathrm { C h a r g e }$ controller efficiency
...
$$
I _ {d c} = \frac {P _ {\text { a   v   e } , \text { p   e   a   k }}}{V _ {d c}} \tag {18}
$$
...
where $N _ { s m } = .$ Number of module in series, $V _ { r m } = 1$ Rated dc voltage of the module
...
From the hydrodynamic characterization simulation in the previous section, the resistance of the boat can be determined at different speeds. Considering a
...
![](images/f00a370fce62075bfcf20c6c9c78a52ca5d59222e248e7ee090ba815dc750b27.jpg)

<details>
<summary>line</summary>

Boat Resistance
| Speed of vessel [knot] | CAHI (N) | Savitsky (N) |
|---|---|---|
| 1 | 480 | 400 |
| 2 | 510 | 460 |
| 3 | 550 | 550 |
...
![](images/fc68eba9f14d46e012b932bcafc3464c516ac517cfe8ebe5b3e996b5357a5df4.jpg)

<details>
<summary>line</summary>

| speed of vessel [knot] | CAHI  | Savitsky |
| ---------------------- | ----- | -------- |
| 1                      | 16.5  | 12.0     |
| 2                      | 12.5  | 11.5     |
| 3                      | 10.5  | 10.8     |
...
![](images/698291f41ec7466215bc1abde8028d4d19789bd43df65bd1d90137f1d18cc051.jpg)
...
maximum design speed of 5 knot, it can be read from Figure 4 that, the corresponding resistance of the boat hull is about 740 N as predicted by the CAHI method. It is safer to use the maximum predicted resistance in order to avoid an under-designed system. Therefore, the power requirement was estimated as the product of the boat resistance and its designed allowable speed (power = $7 4 0 ^ { * } 5 ^ { * } 0 . 5 1 4 4 = 1 . 9 0 3 \mathrm { k W } )$ .
...
$$
E _ {r d} = \frac {E _ {d}}{\eta_ {b} \eta_ {i} \eta_ {c}} = \frac {8 . 9 5 6 \mathrm{kW} \cdot \mathrm{h}}{0 . 9 \times 0 . 9} = 1 1. 0 5 7 \mathrm{kW} \cdot \mathrm{h} / \text {day}
$$

Table 2. PV module specifications. 

<table><tr><td>Parameter</td><td>Symbol</td><td>Value</td></tr><tr><td>Maximum Power</td><td> $P_{\text{max}}$ </td><td>345 W</td></tr><tr><td>Open circuit voltage</td><td> $V_{oc}$ </td><td>47.8 V</td></tr><tr><td>Maximum power point voltage</td><td> $V_{mpp}$ </td><td>38.2 V</td></tr><tr><td>Short circuit current</td><td> $I_{sc}$ </td><td>9.75 A</td></tr><tr><td>Maximum power point current</td><td> $I_{mpp}$ </td><td>9.10 A</td></tr></table>
...
The number of modules in series is 1 since the value of $V _ { r m }$ is greater than $V _ { d c }$

Next, we obtain the number of parallel number of module strings using equation (20). This yielded a total of 10 modules in parallel. Therefore, the total number of solar module for the boat to fully charge the batteries is 10 modules. The sizing of the solar panel for boat design is limited to the available space for installation on the boat. As a result of the limited space available, the panels installed may not be able to fully recharge the battery. As such, the need for additional power source (Land base) is required for charging the batteries.
