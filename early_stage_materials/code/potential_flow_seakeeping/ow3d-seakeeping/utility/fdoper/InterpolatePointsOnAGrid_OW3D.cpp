...
// ====================================================================================
{
  ...
  for (int axis = 0; axis < mg.numberOfDimensions(); axis++)
  {
    ...
    if (!mg.isPeriodic(axis))
    {
      ...
      if ((intLoc + width - 1 > mg.extendedIndexRange(1, axis)) && (mg.boundaryCondition(End, axis) > 0))
      {
        // Point is close to a BC side, one-sided interpolation used.
...
#define X(i0, i1) xp[i0 + xDim0 * (i1)]
  ...
  const realArray &rProject = surfaceGrid ? mpParams.getRealArray(MappingProjectionParameters::r) : Overture::nullRealDistributedArray();
...
#define XPROJECT(i0, i1) xProjectp[i0 + xProjectDim0 * (i1)]
  ...
  for (grid = numberOfComponentGrids - 1; grid >= 0; grid--) // check highest priority grid first
  {
    if (checkTheseGrids != NULL && (*checkTheseGrids)(grid) == 0)
    ...
    unsigned int side_, axis_;
    vector<double> body_axis;
    bool body_found = false;
    for (axis_ = 0; axis_ < mg.numberOfDimensions(); axis_++)
    {
      for (side_ = 0; side_ < 2; side_++)
      {
        if (mapping.getBoundaryCondition(side_, axis_) == 3)
        {
          body_axis.push_back(axis_);
          body_found = true;
        }
      }
    }
    ...
    for (int grid = 0; grid < cgTo.numberOfComponentGrids(); grid++)
    {
      ...
      else
      {
        ...
        { // limit bounds to the dimension array
          ...
          if (ib < ia)
            ib = ia; // what should we do in this case? Just return ?
          Iv[axis] = Range(ia, ib);
        }
      }
...
#ifdef USE_PPP
        realSerialArray center;
        getLocalArrayWithGhostBoundaries(mg.center(), center);
#else
        const realSerialArray &center = mg.center();
#endif
...
#ifdef USE_PPP
      realSerialArray u;
      getLocalArrayWithGhostBoundaries(uTo[grid], u);
#else
